...
while field <= 24 :
	...
	outdir = "/run/media/gmedina/TOSHIBA EXT/DECam2018/Data/Fields_with_uncalibrated_magnitudes/"
	...
	cols = ['id','coord_ra','coord_dec', 'object', 'visit', 'ccdnum', 'MJDObs', 'AIRMASS', 'SEEING', 'EXPTIME', 'base_PsfFlux_flux', 'base_PsfFlux_fluxSigma']
	df = pd.read_csv(indir+'/RR%i/'%field+archivo, usecols=cols)
	...
	# crear columnas a, k
	data_decam = np.loadtxt( '/run/media/gmedina/TOSHIBA EXT/DECam2018/Data/DECAM_zps_%s.dat'%band , dtype='str')
	aas     = data_decam[:,2]
	...
	data_zp_rel = np.loadtxt( '/run/media/gmedina/TOSHIBA EXT/DECam2018/Data/RR%s/zp_files/zp_relativos_%s_%s.dat'%(field,field,band) , dtype='str')
