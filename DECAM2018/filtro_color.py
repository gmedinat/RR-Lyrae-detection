...
files_0 = glob.glob("/run/media/gmedina/TOSHIBA EXT/DECam2018/datos_leftraru/Estrellas_Gen_All_Periods_P4J/*.csv")
...
for f in files:
	...
	#print field, ccdnum_useful
	#print int(field)%2

	if int(field)%2 == 0:
		catalog_file = '/run/media/gmedina/TOSHIBA EXT/DECam2018/Data/Fields_with_uncalibrated_magnitudes/pares/PerCCD/RR%s_ccdnum%s_g.csv'%(field,ccdnum_useful)
	if int(field)%2 != 0:
		catalog_file = '/run/media/gmedina/TOSHIBA EXT/DECam2018/Data/Fields_with_uncalibrated_magnitudes/impares/PerCCD/RR%s_ccdnum%s_g.csv'%(field,ccdnum_useful)
