...
while campo <= 24:
	...
	catalogo_ref = glob.glob("/run/media/gmedina/TOSHIBA EXT/DECam2018/Data/RR%s/RR%s_%s_*_%s.csv"%(campo,campo, epoca_ref,filtro))[0]

	#catalogo_ref = catalogo_ref.replace(indir+'/', '')
	#df_ref = pd.read_csv(catalogo_ref)

	outdir = "/run/media/gmedina/TOSHIBA EXT/DECam2018/Data/RR%s/zp_files"%campo
	...
	for f in files:
		...
		if f[len(f)-5:]=='%s.csv'%filtro and f[len(f)-9:len(f)-6] != 'all':
			...
			if catalogo_ref.replace(indir+'/', '') == f:
				...
				MJD_ref = MJD
				if os.path.exists('/run/media/gmedina/TOSHIBA\ EXT/DECam2018/Data/RR%s/tabla.csv'%campo):
				    os.system('rm /run/media/gmedina/TOSHIBA\ EXT/DECam2018/Data/RR%s/tabla.csv'%campo)
				print '\t \t \t Done!'
			else:
				os.system('java -jar stilts.jar tskymatch2 in1=/run/media/gmedina/TOSHIBA\ EXT/DECam2018/Data/RR%s/%s ifmt1=csv in2=/run/media/gmedina/TOSHIBA\ EXT/DECam2018/Data/RR%s/%s ifmt2=csv out=/run/media/gmedina/TOSHIBA\ EXT/DECam2018/Data/RR%s/tabla.csv ofmt=csv ra1="RA_deg" dec1="DEC_deg" ra2="RA_deg" dec2="DEC_deg" error=1 join=1and2'%(campo, catalogo_ref.replace(indir+'/', ''), campo, f, campo) )
				#abrir tabla
				...
				data_decam = np.loadtxt( '/run/media/gmedina/TOSHIBA EXT/DECam2018/Data/DECAM_zps_%s.dat'%filtro , dtype='str')
				aas     = data_decam[:,2]
