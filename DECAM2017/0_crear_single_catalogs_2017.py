...
while field <= 16 :
	outdir = "/run/media/gmedina/TOSHIBA EXT/DECam2017/Data/Field_%i"%field 
	...
	if band == 'r' or band == 'g': # juntar
		...
		while ccdnum <= 62:
			...
			if ccdnum == 61: # el 61 no esta
				...
				#cols = ['id','coord_ra','coord_dec', 'object', 'visit', 'ccdnum', 'MJDObs', 'base_SdssShape_psf_xx', 'base_SdssShape_psf_yy', 'AIRMASS', 'SEEING', 'EXPTIME', 'base_PsfFlux_flux', 'base_PsfFlux_fluxSigma', 'CCDNUM']
				#df = pd.read_csv(indir+'/RR%s/'%file_n+archivo, usecols=cols)
				archivo = 'Field_%i/RR%i_all_%s_ccd%i.fits'%(field,field,band,ccdnum)
				table = Table.read(indir+'/'+archivo, format='fits')
				df = table.to_pandas()
